  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.reply) {
        ...
        setMessages(prev => [...prev, { id: Date.now().toString(), text: `⚠️ Erro: ${data.error}`, isBot: true }]);
      }
    } catch (e) {
      setMessages(prev => [...prev, { id: Date.now().toString(), text: "⚠️ Erro de conexão com o servidor.", isBot: true }]);
    } finally {
      setLoading(false);
    }
  };
