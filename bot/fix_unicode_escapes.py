...
# Emojis via unicode escapes
text = re.sub(r'emoji_map = \{.*?\}', 'emoji_map = {"backlog": "\\u2744\\ufe0f", "todo": "\\ud83c\\udfaf", "in_progress": "\\u23f3"}', text)
...
text = re.sub(r'return f"[^"]*Transa.*registrad.*\{params\[\'amount\'\]\}!"', 'return f"\\ud83d\\udcb8 Transa\\u00e7\\u00e3o registrada no valor de R$ {params[\'amount\']}!"', text)
text = re.sub(r'return "[^"]*Nenhuma transa.*recente!"', 'return "\\u2728 Nenhuma transa\\u00e7\\u00e3o recente!"', text)
